...
#from factorization_light_transport import frames_dict
from utils import *
import pickle
import glob
import os
...
def get_filenames(path, ext):
    files = natural_sort(glob.glob(path+"*",recursive=False))
...
def load_observations(data_dir, folder_name, dataset, seq_name, skip_frames=0, n_downsample=3, zero_overexp=True,frames_dict = None):
    blacks = None
    blocks_oe = None

    # Read black frames if available
    if 'black' in frames_dict:
        blacks, blacks_oe = load_pgm(os.path.join(data_dir, folder_name, dataset + "_reflector"),
                                     frames_dict['black'][0], frames_dict['black'][1], n_downsample)

    imgs, imgs_oe = load_pgm(os.path.join(data_dir, folder_name, dataset + "_reflector"),
                             frames_dict['impulse'][0], frames_dict['impulse'][1], n_downsample)

    gt_imgs = load_gt(os.path.join(data_dir, folder_name, dataset + "_target"), frames_dict['impulse'][0],
                      frames_dict['impulse'][1], ext="")

    imgs = avg_skip_frames(imgs, skip_frames) #进行帧数的裁剪
    gt_imgs = skip_frames_gt(gt_imgs, skip_frames) #进行帧数的裁剪

    overexp = imgs > (2 ** 16 - 50) #进行阈值判断，但是不知道阈值是怎么得来的
    ...
    if blacks is not None:
        blacks = blacks / 2 ** 14 #每个点的像素值转换为[0,1] 但是为什么是2的14次方
